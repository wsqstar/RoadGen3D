...
Supports any OpenAI-compatible API (GLM, Claude, Gemini via proxy, etc.).
...
class LLMConfigurationError(RuntimeError):
    """Raised when LLM credentials or base URL are missing."""
...
class LLMSettings:
    ...
    def from_env(cls) -> "LLMSettings":
        _maybe_load_dotenv()
        # Prefer GRAPHRAG_ vars; fall back to legacy llm_ vars
        base_url = (
...
class LLMClient:
    """Minimal JSON-capable chat client for LLM OpenAI-compatible endpoints."""
        ...
        *,
        ...
        for attempt in range(1, self.max_retries + 1):
            ...
            try:
                ...
                raise LLMResponseError("LLM response did not include a valid message payload.") from exc
    ...
    def chat_json(
        ...
        *,
        ...
        if not isinstance(payload, dict):
            raise LLMResponseError("Expected the LLM to return a JSON object.")
        return payload
...
def extract_json_payload(text: str) -> Any:
    ...
    for index, char in enumerate(payload_text):
        ...
        try:
            ...
            continue
    raise LLMResponseError("LLM response did not contain a valid JSON payload.")
