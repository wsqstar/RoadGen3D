...
from .knowledge.sidewalk_service import SidewalkKnowledgeService
...
_CATEGORY_TO_COMPONENT = {
    "bench": "public_seating",
    "lamp": "pedestrian_lighting",
    "trash": "waste_receptacle",
    "tree": "street_tree",
    "bus_stop": "bus_stop",
    "mailbox": "mail_service",
...
def _infer_sidewalk_zone_from_band(band: StreetBand | None) -> str:
    if band is None:
        return ""
    name = str(getattr(band, "name", "") or "").lower()
    kind = str(getattr(band, "kind", "") or "").lower()
    if "clear" in name or kind == "clear_path":
        return "pedestrian"
    if "furnish" in name or kind == "furnishing":
        return "amenity"
    if "transit" in name or "parking" in name or kind == "transit_edge":
        return "flex"
    return ""
...
def _evaluate_slot_candidate(
    *,
    ...
    sidewalk_service: Optional[SidewalkKnowledgeService],
) -> Tuple[Optional[Dict[str, object]], Optional[str]]:
    point_xz = (
        float(candidate["point_xz"][0]),
        float(candidate["point_xz"][1]),
    )
    sidewalk_zone = _infer_sidewalk_zone_from_band(slot_band)
    sidewalk_guideline_info: Optional[Dict[str, str]] = None
    ...
    if sidewalk_service and config.sidewalk_constraints_enabled:
        component_key = _CATEGORY_TO_COMPONENT.get(category)
        if component_key and sidewalk_zone:
            guidelines = sidewalk_service.query(zone=sidewalk_zone, component=component_key)
            if not guidelines:
                return None, "sidewalk_rule_block"
            top_guideline = guidelines[0]
            sidewalk_guideline_info = {
                "component": component_key,
                "zone": sidewalk_zone,
                "citation": top_guideline.citation,
                "text": top_guideline.text,
            }
    ...
    if sidewalk_guideline_info:
        resolved_payload["sidewalk_guideline"] = sidewalk_guideline_info
...
def compose_street_scene(
    ...
    sidewalk_service: Optional[SidewalkKnowledgeService] = None
    if config.sidewalk_constraints_enabled and config.sidewalk_knowledge_dir:
        sidewalk_dir = Path(config.sidewalk_knowledge_dir).expanduser().resolve()
        sidewalk_service = SidewalkKnowledgeService(sidewalk_dir)
    ...
    sidewalk_guidance_records: List[Dict[str, str]] = []
    ...
    def _attempt_place_slot(
        ...
        *,
        ...
        for candidate_group in candidate_groups:
            ...
            for candidate in candidate_group:
                ...
                resolved_candidate, blocked_reason = _evaluate_slot_candidate(
                    ...
                    sidewalk_service=sidewalk_service,
                )
                if blocked_reason is not None:
                    blocked_reason_counts[blocked_reason] = blocked_reason_counts.get(blocked_reason, 0) + 1
                    extra_meta = {"repair_phase": bool(repair_phase)}
                    if blocked_reason == "sidewalk_rule_block":
                        extra_meta["sidewalk_zone"] = _infer_sidewalk_zone_from_band(band)
                        extra_meta["sidewalk_component"] = _CATEGORY_TO_COMPONENT.get(category, "")
                    _append_placement_decision_event(
                        decision_events,
        ...
        chosen_guideline = chosen_candidate.get("sidewalk_guideline") or {}
        sidewalk_citation = str(chosen_guideline.get("citation", "") or "")
        ...
        placement = StreetPlacement(
            ...
            **_slot_spatial_kwargs(slot, spatial_ctx),
            sidewalk_guideline=sidewalk_citation,
        )
        placements.append(placement)
        if sidewalk_citation:
            sidewalk_guidance_records.append(
                {
                    "instance_id": placement.instance_id,
                    "asset_id": placement.asset_id,
                    "category": category,
                    "zone": str(chosen_guideline.get("zone", "") or ""),
                    "component": str(chosen_guideline.get("component", "") or ""),
                    "citation": sidewalk_citation,
                }
            )
    ...
    summary_payload = {
        ...
        "sidewalk_guidance": list(sidewalk_guidance_records),
