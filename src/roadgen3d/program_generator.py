...
def _apply_prediction_to_program(base: StreetProgram, prediction: Dict[str, np.ndarray], available_categories: Iterable[str]) -> StreetProgram:
    ...
    return StreetProgram(
        ...
        objective_profile=str(base.objective_profile),
        throughput_requirements=dict(base.throughput_requirements),
        band_bounds={key: dict(value) for key, value in base.band_bounds.items()},
        topology_requirements=dict(base.topology_requirements),
