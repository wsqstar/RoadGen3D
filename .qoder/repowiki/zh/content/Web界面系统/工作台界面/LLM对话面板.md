...
## 更新摘要
**所做更改**
- 更新了对话面板的标签页系统架构，明确其位于第一个标签（Conversation）
- 新增了英雄芯片工作流程指示器与标签页系统的深度集成说明
- 完善了标签页切换逻辑和工作流状态同步机制
- 增强了用户界面导航体验的描述
...
**更新** 对话面板现已集成到全新的标签页系统中，作为第一个标签（Conversation），并与英雄芯片工作流程指示器深度集成，提供直观的工作流状态可视化和智能标签页切换功能。

## 项目结构
对话面板由前端 Web Workbench 与后端服务共同组成：
- 前端（web/workbench）：负责用户交互、消息时间线渲染、参数表单渲染、知识源选择、作业状态轮询与结果展示，现采用标签页系统架构。
- 后端（src/roadgen3d）：负责设计意图解析、RAG 检索、设计草案生成、场景生成与作业调度。

```mermaid
graph TB
subgraph "前端(Web Workbench)"
A["app.ts<br/>主应用与状态管理<br/>标签页系统集成"]
B["api.ts<br/>HTTP请求封装"]
C["types.ts<br/>类型定义"]
D["utils.ts<br/>工具函数"]
E["main.ts<br/>入口"]
F["style.css<br/>标签页样式与英雄芯片样式"]
end
subgraph "后端(RoadGen3D)"
G["design_workflow.py<br/>设计助手服务"]
H["prompts.py<br/>提示词构建"]
I["design_types.py<br/>数据类型"]
J["design_runtime.py<br/>场景生成"]
K["graphrag.py<br/>GraphRAG检索"]
L["pdf_rag.py<br/>PDF RAG检索"]
M["glm_client.py<br/>LLM客户端"]
end
A --> B
A --> C
A --> D
...
- **标签页系统（新增）**
  - 采用五标签页布局：Conversation、Scene Setup、Evidence、Design Draft、Scene Jobs
  - 第一个标签（Conversation）专门负责对话和设计意图澄清
  - 智能标签页切换与工作流状态同步
...
- [app.ts:84-117](file://web/workbench/src/app.ts#L84-L117)

## 架构总览
对话面板采用前后端分离架构：前端通过 HTTP 接口与后端交互，后端内部通过设计助手服务串联 LLM、RAG、场景生成模块。设计意图澄清与参数确认在前端表单中完成，最终提交到后端生成作业并轮询状态。

**更新** 现在采用标签页系统架构，对话面板作为第一个标签（Conversation），与英雄芯片工作流程指示器集成，提供直观的工作流状态可视化。

```mermaid
sequenceDiagram
participant U as "用户"
participant FE as "前端(Web Workbench)<br/>标签页系统"
...
FE-->>U : 渲染参数表单与证据<br/>激活设计草案标签页
U->>FE : 确认参数并创建生成任务
FE->>API : POST /api/scene/jobs
API->>GEN : 生成场景
GEN-->>API : 生成结果
API-->>FE : 轮询作业状态
FE-->>U : 展示viewer链接与导出路径<br/>激活场景作业标签页
end
```
...
### 标签页系统架构与英雄芯片集成
**更新** 对话面板现在位于标签页系统中的第一个标签（Conversation），并与英雄芯片工作流程指示器深度集成：

- **标签页布局**
  - 0=Conversation（对话面板，现为默认激活标签）
  - 1=Scene Setup（场景设置）
  - 2=Evidence（证据展示）
  - 3=Design Draft（设计草案）
  - 4=Scene Jobs（场景作业）
...
- **英雄芯片工作流程指示器**
  - 1. Intent Clarification（意图澄清）
  - 2. GraphRAG Evidence（GraphRAG证据）
  - 3. Parameter Confirmation（参数确认）
  - 4. Scene Job（场景作业）
...
```mermaid
flowchart TD
Workflow["工作流状态"] --> Step1["1. Intent Clarification"]
Workflow --> Step2["2. GraphRAG Evidence"]
Workflow --> Step3["3. Parameter Confirmation"]
Workflow --> Step4["4. Scene Job"]
Step1 --> Tab0["标签页 0: Conversation"]
Step2 --> Tab2["标签页 2: Evidence"]
Step3 --> Tab3["标签页 3: Design Draft"]
Step4 --> Tab4["标签页 4: Scene Jobs"]
Chip1["英雄芯片 1"] --> Tab0
Chip2["英雄芯片 2"] --> Tab2
Chip3["英雄芯片 3"] --> Tab3
Chip4["英雄芯片 4"] --> Tab4
...
- 时间线渲染
  ...
  - 作为第一个标签页，Conversation 标签始终处于激活状态，便于用户随时进行对话。
- 用户输入处理
...
CallAPI --> Stage{"阶段判断"}
Stage -- clarification_required --> RenderClarify["渲染澄清问题<br/>激活Conversation标签"]
Stage -- draft_ready --> RenderDraft["渲染设计草案与证据<br/>激活Design Draft标签"]
RenderClarify --> UpdateWorkflow["更新工作流状态"]
RenderDraft --> UpdateWorkflow
UpdateWorkflow --> SwitchTab["智能切换标签页"]
SwitchTab --> End(["结束"])
```
...
- 设计意图与证据卡片
  ...
  - 参数来源标注使用 tag 标注参数来源（RAG、LLM 推断、用户覆盖、系统默认）。
- **标签页集成**
  - 澄清问题在 Conversation 标签页展示
  - 证据和设计草案在 Evidence 和 Design Draft 标签页分别展示
  - 自动标签页切换确保用户看到正确的信息
...
TabSystem --> DesignIntent : "状态同步"
TabSystem --> RagEvidence : "状态同步"
```
...
- 澄清阶段
  ...
  - **标签页集成**：澄清问题在 Conversation 标签页展示，保持对话连续性。
- 迭代对话
...
```mermaid
sequenceDiagram
participant FE as "前端<br/>Conversation标签"
participant API as "后端API"
participant DA as "设计助手服务"
participant LLM as "LLM客户端"
FE->>API : POST /api/design/draft
...
DA-->>FE : stage=clarification_required
FE-->>用户 : 展示澄清问题<br/>激活Conversation标签
用户->>FE : 回答澄清问题
FE->>API : 再次POST /api/design/draft
API->>DA : draft_design(...)
DA-->>FE : stage=draft_ready
FE-->>用户 : 激活Design Draft标签
```
...
- **标签页集成**
  - 知识源选择在 Conversation 标签页的对话界面中进行
  - 手动知识查询在 Evidence 标签页中进行
...
- 步行安全全龄友好预设
  ...
  - 预设按钮在 Conversation 标签页中，确保用户能够快速访问常用场景。
- 自动生成
...
- 前端状态
  ...
  - **标签页状态**：维护 activeTab 状态，确保标签页与工作流状态同步。
- 缓存机制
  - 后端对设计草案进行缓存，命中时直接返回并提示用户。
  - **智能状态同步**：缓存命中时自动更新英雄芯片状态和标签页激活状态。
...
- 用户体验
  ...
  - **标签页优化**：智能标签页切换，确保用户始终在正确的标签页查看相关信息。
  - **英雄芯片反馈**：实时显示工作流进度，提供清晰的导航指示。
...
- 作业轮询
  ...
  - **标签页集成**：生成完成后自动激活 Scene Jobs 标签页，便于用户查看和管理作业。
...
- **标签页系统**：app.ts 中的标签页管理逻辑与英雄芯片指示器紧密集成。

```mermaid
graph LR
FE["前端(app.ts)<br/>标签页系统"] --> API["后端API"]
...
FE --> STYLE["样式(style.css)<br/>标签页样式"]
```
...
- 生成作业失败
  - 查看作业错误信息，必要时重新创建作业。
- **标签页问题**
  - 标签页不显示：检查 activeTab 状态和标签页切换逻辑
  - 英雄芯片状态不同步：检查 computePipelineStep 和 renderHeroSteps 函数
  - 工作流状态异常：验证 pipelineStepToTab 映射关系
...
## 结论
该对话面板通过清晰的前后端分工、稳健的意图澄清与参数确认流程、灵活的知识源切换与缓存策略，以及全新的标签页系统架构，实现了从自然语言到可编辑参数再到 3D 场景的高效闭环。新增的英雄芯片工作流程指示器与智能标签页切换功能显著提升了用户体验，使用户能够直观地理解工作流进度并快速导航到相应的功能标签页。前端注重用户体验，后端强调可扩展性与稳定性，适合在复杂城市设计场景中快速迭代与验证。
...
- 知识源选择
  ...
  - 知识源选择在 Conversation 标签页中进行，支持实时预览效果。
- 场景上下文
  - 支持布局模式、城市、AOI、参考平面与图模板等上下文配置。
  - 场景设置在 Scene Setup 标签页中进行，提供详细的参数控制。
- 参数表单
  - 前端根据 FIELD_CONFIGS 渲染参数表单，用户可覆盖默认值并查看引用来源。
  - 参数表单在 Design Draft 标签页中展示，支持证据引用和来源追踪。
- **标签页系统扩展**
  - 新增标签页：在 app.ts 中添加新的标签页配置和切换逻辑
  - 工作流集成：更新 computePipelineStep 和 pipelineStepToTab 映射
  - 样式定制：在 style.css 中添加新的标签页样式规则
- 扩展点
