...
面板提供了直观的用户界面，允许用户选择城市、配置 AOI 区域、预览图模板和参考计划，以及进行参数验证和错误处理。通过集成多种数据源和算法，场景设置面板能够为不同需求的用户提供灵活的场景生成解决方案。

## 项目结构

场景设置面板位于 Web 工作台前端应用中，采用模块化的 TypeScript 架构设计：
...
subgraph "数据资源"
I[china_cities.py]
J[graph_templates.py]
K[reference_annotation.py]
end
A --> B
A --> C
A --> D
...
**图表来源**
- [web/workbench/src/app.ts:1-120](file://web/workbench/src/app.ts#L1-L120)
- [src/roadgen3d/services/scene_context_service.py:1-50](file://src/roadgen3d/services/scene_context_service.py#L1-L50)
...
**章节来源**
- [web/workbench/src/app.ts:142-192](file://web/workbench/src/app.ts#L142-L192)
- [web/workbench/src/types.ts:41-47](file://web/workbench/src/types.ts#L41-L47)

## 架构概览

场景设置面板采用分层架构设计，实现了清晰的关注点分离：
...
```mermaid
flowchart TD
A[选择 graph_template 模式] --> B[加载图模板列表]
B --> C{模板加载成功?}
C --> |是| D[显示模板预览]
C --> |否| E[显示错误信息]
...
subgraph "服务层"
D[scene_context_service.py]
E[generation_api.py]
F[design_types.py]
end
subgraph "数据层"
G[china_cities.py]
H[graph_templates.py]
I[reference_annotation.py]
J[osm_ingest.py]
end
A --> B
A --> D
