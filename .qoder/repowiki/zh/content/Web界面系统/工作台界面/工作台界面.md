...
## 更新摘要
**变更内容**
- 完全重构为基于标签页的导航系统，替代原有的堆叠布局
- 新增五个工作流程标签（Conversation、Scene Setup、Evidence、Design Draft、Scene Jobs）
- 集成英雄芯片工作流程指示器，实现自动标签页切换
- 移除原有的配置栏系统
- 增强参数表单功能，支持引文芯片和LLM上下文切换
- 新增内联3D预览功能
...
**更新** 本次更新反映了应用的重大变更：工作台界面完全重构为基于标签页的导航系统，替代原有的堆叠布局。新增五个工作流程阶段（Conversation、Scene Setup、Evidence、Design Draft、Scene Jobs），集成英雄芯片工作流程指示器，实现自动标签页切换和状态管理。移除了原有的配置栏系统，增强了参数表单功能，支持引文芯片和LLM上下文切换。
...
- [web/workbench/src/app.ts:1-1438](file://web/workbench/src/app.ts#L1-L1438)
...
- [web/workbench/src/style.css:1-737](file://web/workbench/src/style.css#L1-L737)
...
**更新** 新增了基于标签页的导航系统、英雄芯片步骤指示器、内联3D预览功能的相关组件。
...
- **新增** 标签页导航系统：基于英雄芯片步骤指示器自动切换标签页，提供清晰的工作流程可视化。

**更新** 新增了基于标签页的导航系统，替代原有的堆叠布局，提供更好的用户体验。
...
W->>W : 自动切换到Scene Jobs标签页
end
W->>U : 展示结果/链接
```

**图表来源**
- [web/workbench/src/app.ts:579-617](file://web/workbench/src/app.ts#L579-L617)
...
- 状态模型
  ...
  - **新增** activeTab：当前激活的标签页索引（0=Conversation, 1=Scene Setup, 2=Evidence, 3=Design Draft, 4=Scene Jobs）
- DOM与事件
  - 初始化渲染多个面板（对话、场景设置、知识检索、证据、草稿、作业）
  - 事件绑定：知识源切换、布局模式切换、城市/模板/参考计划变化、AOI输入、生成按钮点击等
  - **新增** 标签页事件绑定：点击标签页按钮切换内容
  - **新增** 英雄芯片点击事件：跳转到对应工作流程阶段
- 关键流程
  - requestDesignDraft：调用后端草稿接口，更新消息与草稿状态，渲染证据与参数表单
  - createSceneJobFromDraft：构建场景上下文，提交作业，轮询状态并刷新UI
  - **新增** computePipelineStep：计算当前工作流程阶段
  - **新增** pipelineStepToTab：将工作流程阶段映射到标签页索引
  - **新增** switchTab：切换标签页显示状态
  - **新增** renderHeroSteps：根据当前工作流程阶段渲染英雄芯片步骤指示器并自动切换标签页
  - **新增** toggleInlinePreview：控制内联3D预览的显示与隐藏
  - bootstrap：启动时加载知识源、城市、模板、参考计划、最近场景，处理未完成作业
  - pollSceneJob：按间隔轮询作业状态，直到终端态
  - **新增** 同步按钮状态：处理参数表单中的引文芯片和LLM上下文切换
- 错误处理
  - setStatus统一展示状态与错误
  - formatBootstrapError统一格式化启动阶段错误
  - 请求异常捕获并提示

**更新** 新增了基于标签页的导航系统，包括英雄芯片步骤指示器、自动标签页切换和状态管理。
...
subgraph "标签页导航"
AutoTab --> Tab1["Conversation"]
AutoTab --> Tab2["Scene Setup"]
AutoTab --> Tab3["Evidence"]
AutoTab --> Tab4["Design Draft"]
AutoTab --> Tab5["Scene Jobs"]
end
subgraph "新增功能"
HeroSteps --> InlinePreview["内联3D预览"]
InlinePreview --> TogglePreview["toggleInlinePreview() 切换预览"]
end
```

**图表来源**
- [web/workbench/src/app.ts:87-117](file://web/workbench/src/app.ts#L87-L117)
- [web/workbench/src/app.ts:579-617](file://web/workbench/src/app.ts#L579-L617)
- [web/workbench/src/app.ts:99-107](file://web/workbench/src/app.ts#L99-L107)
- [web/workbench/src/app.ts:1217-1241](file://web/workbench/src/app.ts#L1217-L1241)
- [web/workbench/src/app.ts:459-462](file://web/workbench/src/app.ts#L459-L462)

**章节来源**
- [web/workbench/src/app.ts:58-85](file://web/workbench/src/app.ts#L58-L85)
- [web/workbench/src/app.ts:396-462](file://web/workbench/src/app.ts#L396-L462)
- [web/workbench/src/app.ts:464-617](file://web/workbench/src/app.ts#L464-L617)
- [web/workbench/src/app.ts:857-879](file://web/workbench/src/app.ts#L857-L879)
...
- **新增** 标签页样式：.tab-bar、.tab-btn、.tab-content、.tab-stack
- **新增** 内联3D预览样式：#preview-container、.preview-header、.preview-iframe

**更新** 新增了英雄芯片步骤指示器、标签页导航和内联3D预览的样式定义。
...
- 渲染优化
  ...
  - **新增** 标签页切换使用display属性控制显示，避免重新渲染整个界面
- 网络与缓存
  - 草稿接口支持缓存命中，避免重复LLM/RAG计算
  - 知识源状态检查与可用性优先级选择，减少无效请求

**更新** 新增了基于标签页导航系统的性能优化策略。
...
**更新** 新增了基于标签页导航系统的故障排查指南。

**章节来源**
- [web/workbench/src/app.ts:805-826](file://web/workbench/src/app.ts#L805-L826)
...
**更新** 本次更新增强了用户体验，新增的基于标签页的导航系统提供了清晰的工作流程可视化，英雄芯片步骤指示器让用户能够直观地了解当前进度。内联3D预览功能提升了场景预览效率。通过增强的参数表单功能，用户可以更好地理解和管理设计参数的来源和引用关系。
...
- 交互模式
  ...
  - **新增** 标签页导航：点击标签页按钮切换内容区域
  - **新增** 引文芯片：点击展开/收起相关证据详情
  - **新增** LLM上下文切换：点击显示/隐藏LLM推理上下文
- 可访问性
  - 使用语义化标签（h2、label、select、textarea、button）
  - 保持焦点顺序与键盘导航
  - 提供状态提示与错误信息（status-box）
  - **新增** 英雄芯片具有aria-label属性，便于屏幕阅读器识别
  - **新增** 标签页按钮具有data-tab属性，便于辅助技术识别
  - **新增** 引文芯片具有title属性，提供工具提示信息

**更新** 新增了基于标签页导航系统的交互模式和可访问性支持。

**章节来源**
- [web/workbench/src/app.ts:396-410](file://web/workbench/src/app.ts#L396-L410)
- [web/workbench/src/style.css:107-131](file://web/workbench/src/style.css#L107-L131)

### 响应式设计
- 大屏：双栏布局（左侧对话/场景设置，右侧知识/草稿/作业）
- 小屏：单栏布局，网格自适应，面板间距与内边距调整
- **新增** 标签页按钮在小屏设备上自动滚动，支持触摸滑动
- **新增** 英雄芯片在小屏设备上自动换行显示

**更新** 标签页导航的响应式设计优化。

**章节来源**
- [web/workbench/src/style.css:656-737](file://web/workbench/src/style.css#L656-L737)
...
- **新增** 标签页样式支持自定义激活状态和悬停效果

**更新** 英雄芯片步骤指示器和标签页导航的样式定制支持。
...
- **新增** 标签页导航系统扩展
  - 在app.ts中扩展pipelineStepToTab函数以支持新的标签页映射
  - 在style.css中扩展.tab-btn样式以支持新的标签页外观
- **新增** 内联3D预览扩展
  - 在app.ts中扩展toggleInlinePreview函数以支持新的预览模式
  - 在style.css中扩展预览相关样式
- **新增** 参数表单功能扩展
  - 在renderParameterForm中支持新的字段类型和验证逻辑
  - 在buildCitationHtml和buildLlmContextHtml中扩展引用和上下文展示

**更新** 新增了基于标签页导航系统的扩展指南。
...
- [web/workbench/src/app.ts:94-97](file://web/workbench/src/app.ts#L94-L97)
- [web/workbench/src/style.css:672-737](file://web/workbench/src/style.css#L672-L737)