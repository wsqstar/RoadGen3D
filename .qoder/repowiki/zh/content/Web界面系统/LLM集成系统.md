...
## 更新摘要
**变更内容**
- GLM客户端重大改进：新增指数退避重试机制、HTTP 429速率限制处理、双凭证管理系统
- 增强的错误处理和异常传播机制
- 可配置的重试参数（最大重试次数、基础延迟时间）
- 改进的速率限制处理，支持标准Retry-After头和自定义错误载荷
- 双环境变量凭证系统：GRAPHRAG_API_KEY/GRAPHRAG_API_BASE（首选）和传统glm_base_url/key（回退）
...
**更新** 系统现已新增图感知设计功能，能够基于道路网络图结构和可选的参考底图自动生成初始设计参数，显著提升了设计工作的效率和质量。同时，GLM客户端经过重大改进，具备了更强的鲁棒性和可靠性。
