# 开发者指南

<cite>
**本文档引用的文件**
- [README.md](file://README.md)
- [Makefile](file://Makefile)
- [requirements-m1.txt](file://requirements-m1.txt)
- [requirements-m2.txt](file://requirements-m2.txt)
- [requirements-m5.txt](file://requirements-m5.txt)
- [requirements-ui.txt](file://requirements-ui.txt)
- [scripts/test_llm_api.sh](file://scripts/test_llm_api.sh)
- [scripts/snapshot_diff.py](file://scripts/snapshot_diff.py)
- [scripts/enrich_street_furniture_manifest.py](file://scripts/enrich_street_furniture_manifest.py)
- [tests/test_auto_eval.py](file://tests/test_auto_eval.py)
- [metaurban/setup.py](file://metaurban/setup.py)
- [metaurban/environment.yml](file://metaurban/environment.yml)
- [src/roadgen3d/__init__.py](file://src/roadgen3d/__init__.py)
- [web/api/main.py](file://web/api/main.py)
- [web/viewer/index.html](file://web/viewer/index.html)
- [web/workbench/package.json](file://web/workbench/package.json)
- [scripts/m1_00_check_env.py](file://scripts/m1_00_check_env.py)
- [scripts/snapshot_diff.py](file://scripts/snapshot_diff.py)
- [scripts/enrich_street_furniture_manifest.py](file://scripts/enrich_street_furniture_manifest.py)
- [src/roadgen3d/asset_scale.py](file://src/roadgen3d/asset_scale.py)
- [src/roadgen3d/program_generator.py](file://src/roadgen3d/program_generator.py)
- [src/roadgen3d/layout_features.py](file://src/roadgen3d/layout_features.py)
- [src/roadgen3d/decoder_shapee.py](file://src/roadgen3d/decoder_shapee.py)
- [src/roadgen3d/llm/design_workflow.py](file://src/roadgen3d/llm/design_workflow.py)
- [src/roadgen3d/services/design_runtime.py](file://src/roadgen3d/services/design_runtime.py)
</cite>

## 更新摘要
**所做更改**
- 新增uv现代Python包管理器作为主要依赖管理解决方案的详细说明
- 新增LLM API测试脚本的使用方法和配置指南
- 更新开发环境搭建步骤以反映新的依赖管理流程
- 增强测试策略章节，包含LLM API连接性测试和自动化评估工具测试
- 完善自动化测试指导，涵盖新的评估工具和数据处理脚本

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖分析](#依赖分析)
7. [性能考虑](#性能考虑)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录](#附录)

## 简介
本指南面向 RoadGen3D 的开发者，覆盖从环境搭建、代码规范与最佳实践、测试策略、贡献流程、调试与性能分析，到扩展开发（插件、自定义资产后端、新增管道阶段）以及 CI/CD 与部署自动化。内容基于仓库中的实际文件进行梳理，确保可操作性与一致性。

## 项目结构
RoadGen3D 采用多语言混合架构：核心 Python 库位于 src/roadgen3d，Web 后端服务通过 FastAPI 提供 API，前端工作台与查看器分别由 Vite/React 和 Three.js 驱动；数据与知识库资源位于 data/ 与 knowledge/；脚本目录 scripts/ 提供各里程碑的命令行工具；tests/ 提供端到端与集成测试。

```mermaid
graph TB
subgraph "后端服务"
API["FastAPI 应用<br/>web/api/main.py"]
SVC["设计助手服务<br/>设计草稿/生成/作业管理"]
end
subgraph "前端"
WB["工作台(Vite)<br/>web/workbench/package.json"]
VIEW["查看器(Three.js)<br/>web/viewer/index.html"]
end
subgraph "核心库"
CORE["RoadGen3D 核心包<br/>src/roadgen3d/__init__.py"]
end
subgraph "数据与资源"
DATA["资产清单/材料<br/>data/"]
KNOW["知识库(PDF/GraphRAG)<br/>knowledge/"]
MODELS["预训练模型<br/>models/"]
end
subgraph "工具与脚本"
SCRIPTS["里程碑脚本<br/>scripts/*"]
TESTS["测试套件<br/>tests/*"]
MAKE["Makefile 构建目标"]
end
API --> SVC
SVC --> CORE
WB --> API
VIEW --> API
CORE --> DATA
CORE --> KNOW
CORE --> MODELS
SCRIPTS --> CORE
TESTS --> API
TESTS --> CORE
MAKE --> API
MAKE --> WB
MAKE --> VIEW
```

**图表来源**
- [web/api/main.py:81-267](file://web/api/main.py#L81-L267)
- [src/roadgen3d/__init__.py:1-295](file://src/roadgen3d/__init__.py#L1-L295)
- [Makefile:1-107](file://Makefile#L1-L107)
- [web/workbench/package.json:1-16](file://web/workbench/package.json#L1-L16)
- [web/viewer/index.html:1-13](file://web/viewer/index.html#L1-L13)

**章节来源**
- [README.md:107-130](file://README.md#L107-L130)
- [Makefile:13-107](file://Makefile#L13-L107)

## 核心组件
- 设计助手服务与 API：提供草稿生成、场景生成、作业队列、最近场景列表、知识库重建与检索、参考图与图模板查询等接口。
- 核心库导出：集中暴露类型、解码器、嵌入器、布局求解器、程序生成器、OSM/POI 规则、评估指标、渲染与导出等能力。
- 前端工作台与查看器：分别提供交互式生成界面与 3D 场景浏览。
- **新增** LLM API测试工具：提供LLM连接性测试功能，支持模型列表查询和API连通性验证。
- **新增** 自动化评估工具：包含 snapshot-diff 自动化评估脚本和街道家具清单增强工具，支持完整的迭代评估和数据处理流程。

**章节来源**
- [web/api/main.py:81-267](file://web/api/main.py#L81-L267)
- [src/roadgen3d/__init__.py:1-295](file://src/roadgen3d/__init__.py#L1-L295)
- [web/workbench/package.json:1-16](file://web/workbench/package.json#L1-L16)
- [web/viewer/index.html:1-13](file://web/viewer/index.html#L1-L13)
- [scripts/test_llm_api.sh:1-68](file://scripts/test_llm_api.sh#L1-L68)

## 架构总览
系统采用"文本提示 → 资产检索 → 街道程序/约束集 → 布局求解 → 网格导出"的神经符号管线，并通过 FastAPI 提供异步作业模式，前端工作台与查看器作为可视化与交互入口。

```mermaid
sequenceDiagram
participant Dev as "开发者/用户"
participant WB as "工作台前端"
participant API as "FastAPI 后端"
participant SVC as "设计助手服务"
participant CORE as "核心库"
participant FS as "文件系统"
Dev->>WB : 打开工作台
WB->>API : POST /api/scene/jobs (提交生成作业)
API->>SVC : create_scene_job(draft, context)
SVC->>CORE : compose_street_scene(...)
CORE-->>SVC : 返回布局/网格路径
SVC-->>API : 返回作业状态
API-->>WB : 返回作业ID/状态
Dev->>WB : 查看最近场景
WB->>API : GET /api/scenes/recent
API->>SVC : list_recent_scenes()
SVC-->>API : 返回场景列表
API-->>WB : 返回场景元数据
Dev->>WB : 打开查看器
WB->>API : GET /api/reference-plans 或 /api/graph-templates
API->>SVC : 列表/图片资源
SVC-->>API : 返回资源URL
API-->>WB : 返回资源
```

**图表来源**
- [web/api/main.py:188-221](file://web/api/main.py#L188-L221)
- [web/api/main.py:106-142](file://web/api/main.py#L106-L142)
- [web/api/main.py:217-221](file://web/api/main.py#L217-L221)

## 详细组件分析

### 组件A：Web API（FastAPI）
- 功能要点
  - 健康检查、城市列表、参考方案与图模板列表及图片获取。
  - 设计草稿、场景生成、作业队列、最近场景、知识库重建与检索、场景评估。
  - 使用 Pydantic 模型校验请求载荷，统一返回 make_json_safe 包装。
- 错误处理
  - 对 LLM 配置错误、响应错误、运行时错误进行分类处理并返回相应 HTTP 状态码。
- 接口形态
  - 异步作业模式为主，支持同步生成与历史场景查询。

```mermaid
sequenceDiagram
participant Client as "客户端"
participant API as "FastAPI 应用"
participant SVC as "设计助手服务"
participant CORE as "核心库"
Client->>API : POST /api/scene/jobs
API->>SVC : create_scene_job(...)
SVC->>CORE : compose_street_scene(...)
CORE-->>SVC : 布局/网格路径
SVC-->>API : 作业状态
API-->>Client : 作业ID/状态
Client->>API : GET /api/scenes/recent
API->>SVC : list_recent_scenes()
SVC-->>API : 场景列表
API-->>Client : 场景元数据
```

**图表来源**
- [web/api/main.py:188-221](file://web/api/main.py#L188-L221)
- [web/api/main.py:217-221](file://web/api/main.py#L217-L221)

**章节来源**
- [web/api/main.py:81-267](file://web/api/main.py#L81-L267)

### 组件B：前端工作台与查看器
- 工作台
  - 使用 Vite + TypeScript 开发，提供本地开发服务器与构建脚本。
- 查看器
  - 基于 Three.js 的 3D 场景浏览页面，通过模块入口加载应用逻辑。

**章节来源**
- [web/workbench/package.json:1-16](file://web/workbench/package.json#L1-L16)
- [web/viewer/index.html:1-13](file://web/viewer/index.html#L1-L13)

### 组件C：核心库导出与类型体系
- 导出清单
  - 包含嵌入器、解码器、索引存储、布局策略与求解器、程序生成器、OSM/POI 规则、评估指标、渲染与导出等。
- 类型与结果
  - 提供丰富的 Pydantic/数据类类型用于参数、结果与中间表示，便于前后端契约一致。

**章节来源**
- [src/roadgen3d/__init__.py:1-295](file://src/roadgen3d/__init__.py#L1-L295)

### 组件D：环境检查与依赖
- 现代Python包管理
  - **更新** 使用uv作为主要依赖管理工具，通过`uv sync`安装所有依赖，替代传统pip方法。
  - 支持离线模式安装，通过`--local-files-only`参数避免在线下载。
- 环境检查脚本
  - 输出 Python 版本、平台、已安装包版本、CUDA/MPS 可用性等报告，便于定位运行时问题。
- 依赖声明
  - M1/M2/M5/UI 分层依赖文件，明确各模块所需第三方库范围。
- MetaUrban 安装
  - setup.py 与 environment.yml 提供打包与 Conda 环境配置示例。

**章节来源**
- [README.md:23-27](file://README.md#L23-L27)
- [scripts/m1_00_check_env.py:1-79](file://scripts/m1_00_check_env.py#L1-L79)
- [requirements-m1.txt:1-7](file://requirements-m1.txt#L1-L7)
- [requirements-m2.txt:1-4](file://requirements-m2.txt#L1-L4)
- [requirements-m5.txt:1-5](file://requirements-m5.txt#L1-L5)
- [requirements-ui.txt:1-12](file://requirements-ui.txt#L1-L12)
- [metaurban/setup.py:1-130](file://metaurban/setup.py#L1-L130)
- [metaurban/environment.yml:1-16](file://metaurban/environment.yml#L1-L16)

### 组件E：测试策略与测试套件
- 测试范围
  - 单元测试：针对具体函数或小模块的行为验证。
  - 集成测试：跨模块协作（如 API 与服务），验证端到端流程。
  - 端到端测试：覆盖完整工作流，包括草稿生成、场景生成、作业队列、最近场景、知识库检索与重建等。
- **新增** LLM API连接性测试
  - 提供`scripts/test_llm_api.sh`脚本，支持LLM API连通性测试、模型列表查询和错误诊断。
  - 支持通过环境变量配置API密钥和基础URL。
- 测试实现
  - 使用 FastAPI TestClient 与自定义 FakeService，模拟真实服务行为，断言响应结构与业务规则。
- 关键测试点
  - 草稿阶段与澄清阶段的返回结构。
  - OSM 场景上下文缺失时的错误处理。
  - 知识源默认值与切换。
  - 参考方案与图模板的列表与图片访问。
  - 注解转换为图谱的正确性与派生拓扑信息。
  - **新增** LLM API测试：验证API密钥配置、模型可用性和响应格式。
- **新增** 自动化评估测试
  - snapshot-diff 脚本的单元测试，验证配置差异计算、HTML 报告生成、评分图表绘制等功能。
  - 表面测试场景验证输出结构、配置差异、比较图像和 HTML 报告的完整性。

```mermaid
flowchart TD
Start(["开始测试"]) --> Draft["POST /api/design/draft"]
Draft --> Gen["POST /api/design/generate"]
Gen --> Job["POST /api/scene/jobs"]
Job --> ListJobs["GET /api/scene/jobs"]
ListJobs --> GetJob["GET /api/scene/jobs/{job_id}"]
GetJob --> Recent["GET /api/scenes/recent"]
Recent --> Rebuild["POST /api/knowledge/rebuild"]
Rebuild --> Sources["GET /api/knowledge/sources"]
Sources --> Search["POST /api/knowledge/search"]
Search --> RefPlans["GET /api/reference-plans"]
RefPlans --> RefImg["GET /api/reference-plans/{id}/image"]
RefImg --> GraphTemplates["GET /api/graph-templates"]
GraphTemplates --> GraphImg["GET /api/graph-templates/{id}/image"]
GraphImg --> Convert["POST /api/reference-annotations/convert"]
Convert --> LLMTest["LLM API测试"]
LLMTest --> OSMCheck{"OSM 上下文有效?"}
OSMCheck --> |否| Err["返回 400 错误"]
OSMCheck --> |是| Pass["通过"]
Pass --> End(["结束"])
Err --> End
```

**图表来源**
- [tests/test_design_api.py:183-523](file://tests/test_design_api.py#L183-L523)
- [scripts/test_llm_api.sh:1-68](file://scripts/test_llm_api.sh#L1-L68)

**章节来源**
- [tests/test_design_api.py:1-523](file://tests/test_design_api.py#L1-L523)
- [tests/test_snapshot_diff.py:1-302](file://tests/test_snapshot_diff.py#L1-L302)
- [scripts/test_llm_api.sh:1-68](file://scripts/test_llm_api.sh#L1-L68)

### 组件F：资产缩放与标准化系统
- 资产缩放系统
  - 支持 canonical_v1 和 native_raw 两种模式，提供树、长椅、路灯、垃圾桶等8种标准资产的缩放优先级。
  - 自动计算主次维度匹配，限制缩放范围，支持回退机制。
- 资产规模统计
  - 汇总场景中各类资产的缩放分布，计算中位数、最小值、最大值和回退次数。

**章节来源**
- [src/roadgen3d/asset_scale.py:1-155](file://src/roadgen3d/asset_scale.py#L1-L155)

### 组件G：程序生成器与布局策略
- 程序生成器
  - 基于神经网络的街道程序生成，支持样本按场景分割，确保训练/验证集的场景独立性。
  - 输出带宽、类别计数、右侧预留、目标权重等预测结果。
- 布局特征工程
  - 上下文感知特征，包含槽位位置、道路宽度、人行道宽度、密度、排名等。
  - 考虑已放置资产数量、唯一性比率、剩余可用性等上下文信息。

**章节来源**
- [src/roadgen3d/program_generator.py:477-505](file://src/roadgen3d/program_generator.py#L477-L505)
- [src/roadgen3d/layout_features.py:89-127](file://src/roadgen3d/layout_features.py#L89-L127)

### 组件H：ShapeE 解码器增强
- 解码器优化
  - 支持跳过体素转换直接返回网格，避免精度损失和额外计算。
  - 失败时自动降级到基础解码器，保留错误信息。
- 错误处理
  - 严格模式下抛出异常，非严格模式下继续执行并标记回退。

**章节来源**
- [src/roadgen3d/decoder_shapee.py:213-244](file://src/roadgen3d/decoder_shapee.py#L213-L244)

### 组件I：设计工作流与缓存机制
- 设计工作流
  - LLM 意图解析、RAG 搜索、证据融合、草稿生成的完整流水线。
  - 支持中英文查询翻译、参数缺失补全、缓存优化。
- 缓存系统
  - 基于 SHA-256 的草稿缓存，避免重复计算，提升响应速度。

**章节来源**
- [src/roadgen3d/llm/design_workflow.py:63-494](file://src/roadgen3d/llm/design_workflow.py#L63-L494)

### 组件J：场景生成运行时
- 配置构建
  - 从设计草稿构建场景配置，合并默认值和用户覆盖。
  - 支持样式预设和美学模式的配置。
- 生成选项
  - 统一的场景生成选项处理，支持多种设备和导出格式。

**章节来源**
- [src/roadgen3d/services/design_runtime.py:60-148](file://src/roadgen3d/services/design_runtime.py#L60-L148)

### 组件K：自动化评估工具链
- **新增** snapshot-diff 自动化评估脚本
  - 支持单查询的完整迭代评估流程，生成可视化的对比报告。
  - 自动构建 FAISS 索引、运行迭代控制器、渲染预览视图、生成配置差异和评分图表。
  - 输出 HTML 报告、JSON 评估报告和迭代日志。
- **新增** 街道家具清单增强工具
  - 从原始清单创建标准化的街道家具清单，添加文本描述、占位符潜在文件、分类标准化等字段。
  - 支持从注释文件中提取分类信息，计算网格面数，生成质量注释。
  - 提供占位符潜在文件的创建和管理。

**章节来源**
- [scripts/snapshot_diff.py:1-608](file://scripts/snapshot_diff.py#L1-L608)
- [scripts/enrich_street_furniture_manifest.py:1-195](file://scripts/enrich_street_furniture_manifest.py#L1-L195)

### 组件L：LLM API测试工具
- **新增** LLM API连接性测试
  - 提供`scripts/test_llm_api.sh`脚本，支持LLM API连通性测试、模型列表查询和错误诊断。
  - 自动加载`.env`文件中的API配置，支持多种测试模式。
  - 支持指定测试模型和列出可用模型。
- 测试功能
  - 验证API密钥配置是否正确
  - 检查API端点可达性
  - 获取可用模型列表
  - 验证基本的聊天接口响应

**章节来源**
- [scripts/test_llm_api.sh:1-68](file://scripts/test_llm_api.sh#L1-L68)

## 依赖分析
- 运行时依赖
  - Python 3.11+（macOS arm64 已验证），NumPy、PyTorch、Transformers、FAISS、Trimesh、Scikit-image、PyGltflib、Shapely、PyProj、Requests、Pillow、FastAPI、Uvicorn、HTTPX、Pydantic 等。
- 现代依赖管理
  - **更新** 使用uv作为主要依赖管理工具，通过`uv sync`安装所有依赖，支持离线模式和增量安装。
  - 支持分层依赖管理，M1/M2/M5/UI 层分别对应不同的功能模块。
- 构建与运行
  - Makefile 提供一键启动 API、工作台、查看器与知识库构建、收集数据、训练策略、工程评估等目标。
  - **新增** snapshot-diff 目标支持自动化评估流程。
- 前端依赖
  - Vite、TypeScript、React 生态，通过 npm 安装与开发。

```mermaid
graph LR
UV["uv 包管理器"] --> PY["Python 3.11+"]
PY --> M1["M1 依赖<br/>requirements-m1.txt"]
PY --> M2["M2 依赖<br/>requirements-m2.txt"]
PY --> M5["M5 依赖<br/>requirements-m5.txt"]
PY --> UI["UI 依赖<br/>requirements-ui.txt"]
M1 --> CORE["核心库<br/>src/roadgen3d/*"]
M2 --> CORE
M5 --> CORE
UI --> API["FastAPI 应用<br/>web/api/main.py"]
API --> WB["工作台<br/>web/workbench/*"]
API --> VIEW["查看器<br/>web/viewer/*"]
```

**图表来源**
- [requirements-m1.txt:1-7](file://requirements-m1.txt#L1-L7)
- [requirements-m2.txt:1-4](file://requirements-m2.txt#L1-L4)
- [requirements-m5.txt:1-5](file://requirements-m5.txt#L1-L5)
- [requirements-ui.txt:1-12](file://requirements-ui.txt#L1-L12)
- [Makefile:1-107](file://Makefile#L1-L107)

**章节来源**
- [README.md:33-56](file://README.md#L33-L56)
- [Makefile:13-107](file://Makefile#L13-L107)

## 性能考虑
- 计算设备选择
  - 通过运行时设备解析函数选择后端与设备，建议在具备 CUDA/MPS 的机器上优先使用 GPU 加速。
- 检索与解码
  - FAISS 内积搜索与 CLIP 文本编码为性能瓶颈之一，建议合理设置 TopK 与批量大小。
- 渲染与导出
  - 网格导出默认使用 Marching Cubes，调试可回退到 cubes；输出格式支持 GLB（显示）与 PLY（调试）。
- 评估指标
  - 工程评估包含多样性、丢槽率、重叠率、命中率与延迟等指标，可用于回归监控。
- **新增** 评估工具性能
  - snapshot-diff 脚本支持离线模式，可避免在线模型下载。
  - 图像处理和图表生成需要额外的内存和 CPU 资源。
- **新增** LLM API性能
  - API测试脚本支持超时配置和重试机制。
  - 模型列表查询可帮助选择最适合的模型。

**章节来源**
- [README.md:145-193](file://README.md#L145-L193)
- [src/roadgen3d/__init__.py:246-250](file://src/roadgen3d/__init__.py#L246-L250)
- [scripts/test_llm_api.sh:46-67](file://scripts/test_llm_api.sh#L46-L67)

## 故障排查指南
- 现代依赖管理
  - **更新** 使用uv进行依赖安装，通过`uv sync`解决依赖冲突；如遇问题，可使用`uv pip install`进行传统pip安装。
  - 检查uv版本兼容性，确保使用最新稳定版本。
- 环境准备
  - 使用环境检查脚本生成报告，确认 Python 版本、平台、包版本与 CUDA/MPS 可用性。
- 依赖安装
  - 按顺序安装 M1/M2/UI 依赖；若需 MetaUrban 环境，可参考 setup.py 与 environment.yml。
- API 服务
  - 若端口占用，Makefile 中的目标会检测端口占用并提示；必要时先释放端口再启动。
- 知识库
  - 使用知识库构建目标从 PDF 构建 GraphRAG 索引；若检索异常，检查知识源可用性与重建日志。
- OSM 场景上下文
  - 当布局模式为 osm 时必须提供 AOI 边界框，否则会触发运行时错误。
- **新增** LLM API故障排查
  - 使用`scripts/test_llm_api.sh`测试API连通性，检查`.env`文件配置。
  - 验证API密钥是否正确设置，支持`GRAPHRAG_API_KEY`和`GRAPHRAG_API_BASE`环境变量。
  - 使用`--list`参数获取可用模型列表，确认模型配置正确。
- **新增** 评估工具故障排查
  - snapshot-diff 脚本需要有效的 CLIP 模型目录和资产清单文件。
  - 图像处理失败时检查 Pillow 安装和图像文件完整性。
  - 图表生成失败时检查 matplotlib 安装和绘图权限。

**章节来源**
- [README.md:23-27](file://README.md#L23-L27)
- [scripts/m1_00_check_env.py:1-79](file://scripts/m1_00_check_env.py#L1-L79)
- [Makefile:39-67](file://Makefile#L39-L67)
- [tests/test_design_api.py:457-471](file://tests/test_design_api.py#L457-L471)
- [scripts/test_llm_api.sh:14-31](file://scripts/test_llm_api.sh#L14-L31)

## 结论
本指南提供了 RoadGen3D 的开发与运维全景：从环境搭建、依赖管理、测试策略到 API 使用、前端交互与核心库能力。**更新**后的环境搭建流程采用uv现代Python包管理器，提供更快、更可靠的依赖安装体验。新增的LLM API测试工具和自动化评估工具链进一步增强了系统的可测试性和质量保证能力。建议在开发过程中遵循统一的类型与契约、保持测试覆盖率，并利用评估指标持续优化管线性能与质量。

## 附录

### A. 开发环境搭建步骤
- 克隆仓库并初始化子模块
- **更新** 安装uv包管理器：`curl -LsSf https://astral.sh/uv/install.sh | sh`
- **更新** 使用uv同步依赖：`uv sync`
- 安装前端依赖：工作台与查看器分别执行安装
- 下载CLIP模型（离线模式）：`huggingface-cli download openai/clip-vit-base-patch32 --local-dir models/clip-vit-base-patch32`
- 启动开发环境：使用 Makefile 的 dev 目标一键启动 API、工作台与查看器

**章节来源**
- [README.md:17-36](file://README.md#L17-L36)
- [Makefile:29-34](file://Makefile#L29-L34)

### B. 代码规范与最佳实践
- 模块组织
  - 核心库集中导出公共 API，避免直接导入内部实现细节。
- 命名约定
  - 类型与结果对象使用清晰的名词短语；服务方法使用动宾结构。
- 注释与契约
  - 使用 Pydantic 模型定义输入/输出契约，配合文档字符串说明用途与边界条件。
- 错误处理
  - 将 LLM 配置错误、响应错误与运行时错误分类处理，返回明确的 HTTP 状态码与错误信息。

**章节来源**
- [src/roadgen3d/__init__.py:1-295](file://src/roadgen3d/__init__.py#L1-L295)
- [web/api/main.py:81-267](file://web/api/main.py#L81-L267)

### C. 测试策略与使用方法
- 单元测试
  - 针对独立函数与小模块进行验证，确保边界条件与异常路径覆盖。
- 集成测试
  - 通过 TestClient 与 FakeService 模拟服务层，验证 API 与服务交互。
- 端到端测试
  - 覆盖草稿生成、场景生成、作业队列、最近场景、知识库检索与重建、参考方案与图模板访问、注解转换等完整流程。
- **新增** LLM API测试
  - 使用`scripts/test_llm_api.sh`测试LLM API连通性，支持多种测试模式。
  - 验证API密钥配置、模型可用性和基本响应格式。
  - 支持离线模式测试，避免网络依赖。
- **新增** 自动化评估测试
  - 使用 mock LLM 模式运行 snapshot-diff 管道，验证输出结构、配置差异、比较图像和 HTML 报告。
  - 测试配置差异计算的准确性，验证 HTML 报告的完整性和有效性。

**章节来源**
- [tests/test_design_api.py:1-523](file://tests/test_design_api.py#L1-L523)
- [tests/test_snapshot_diff.py:1-302](file://tests/test_snapshot_diff.py#L1-L302)
- [scripts/test_llm_api.sh:1-68](file://scripts/test_llm_api.sh#L1-L68)

### D. 贡献流程
- 分支策略
  - 建议采用功能分支开发，完成后合并至主干。
- 提交规范
  - 提交信息应简洁明确，描述变更目的与影响范围。
- Pull Request
  - PR 需包含测试用例与变更说明，确保通过 CI 与代码审查。

### E. 调试技巧与性能分析
- 设备与后端
  - 使用设备解析函数确认后端与设备选择，优先启用 GPU。
- 指标与报告
  - 工程评估指标用于回归监控，结合日志与报告定位性能瓶颈。
- 环境诊断
  - 使用环境检查脚本输出系统与依赖状态，辅助问题定位。
- **新增** LLM API调试
  - 使用`scripts/test_llm_api.sh`进行API连通性测试，检查配置文件和网络连接。
  - 支持模型列表查询，帮助选择最适合的模型。
  - 验证响应格式和HTTP状态码。
- **新增** 评估工具调试
  - 使用 snapshot-diff 脚本的详细输出信息定位问题。
  - 检查中间文件的生成状态，逐步排除故障点。

**章节来源**
- [src/roadgen3d/__init__.py:277-282](file://src/roadgen3d/__init__.py#L277-L282)
- [README.md:184-193](file://README.md#L184-L193)
- [scripts/m1_00_check_env.py:1-79](file://scripts/m1_00_check_env.py#L1-L79)
- [scripts/test_llm_api.sh:33-40](file://scripts/test_llm_api.sh#L33-L40)

### F. 扩展开发指南
- 插件系统
  - 可在核心库中新增模块并通过 __all__ 导出，保持向后兼容。
- 自定义资产后端
  - 通过资产记录与索引存储接口扩展新的资产来源与检索策略。
- 新增管道阶段
  - 在现有管线（检索/布局/导出）中插入新阶段，确保输入/输出与类型一致。
- **新增** LLM API扩展
  - 可以扩展LLM API测试脚本以支持新的API端点和认证方式。
  - 支持自定义模型配置和响应格式验证。
- **新增** 评估工具扩展
  - 可以扩展 snapshot-diff 脚本以支持新的评估指标和可视化方式。
  - 街道家具清单增强工具可以扩展分类映射和质量评估标准。

**章节来源**
- [src/roadgen3d/__init__.py:1-295](file://src/roadgen3d/__init__.py#L1-L295)
- [scripts/test_llm_api.sh:1-68](file://scripts/test_llm_api.sh#L1-L68)

### G. 持续集成与部署自动化
- Makefile 目标
  - 提供一键启动、知识库构建、数据收集、策略训练与工程评估等自动化脚本。
  - **新增** snapshot-diff 目标支持自动化评估流程。
- 前端构建
  - Vite 提供开发与生产构建脚本，便于本地与 CI 环境复现。

**章节来源**
- [Makefile:13-107](file://Makefile#L13-L107)
- [web/workbench/package.json:6-10](file://web/workbench/package.json#L6-L10)

### H. 资产缩放系统最佳实践
- 缩放模式选择
  - canonical_v1：基于标准尺寸的规范化缩放，适合大规模场景一致性。
  - native_raw：保持原生尺寸，适合特定艺术风格需求。
- 缩放优先级
  - 树类资产优先高度，次要考虑树冠宽度。
  - 长椅优先宽度，次要考虑高度。
  - 路灯优先高度，无次要维度。
- 回退机制
  - 当无法确定主维度时自动回退到原生尺寸。
  - 统计回退次数，监控缩放一致性。

**章节来源**
- [src/roadgen3d/asset_scale.py:9-58](file://src/roadgen3d/asset_scale.py#L9-L58)
- [tests/test_m3_street_compose.py:429-450](file://tests/test_m3_street_compose.py#L429-L450)

### I. 程序生成器训练策略
- 数据分割
  - 基于场景ID的哈希值进行随机分割，确保训练/验证集的场景独立性。
  - 支持可调的训练比例参数。
- 训练配置
  - 统一的训练配置管理，支持温度参数调节和检查点加载。
- 预测输出
  - 网络输出经过激活函数处理，确保物理意义的有效性。

**章节来源**
- [src/roadgen3d/program_generator.py:491-505](file://src/roadgen3d/program_generator.py#L491-L505)

### J. 设计工作流缓存优化
- 缓存键生成
  - 基于用户输入和知识源的标准化处理，确保相同输入得到相同缓存键。
- 缓存版本控制
  - 版本化的缓存文件，支持缓存失效和升级。
- 性能收益
  - 显著减少重复计算，提升草稿生成响应速度。

**章节来源**
- [src/roadgen3d/llm/design_workflow.py:402-494](file://src/roadgen3d/llm/design_workflow.py#L402-L494)

### K. 环境变量与配置管理
- 环境变量配置
  - 项目使用标准的环境变量配置方式，支持不同环境下的灵活部署。
  - **新增** LLM API配置：支持`GRAPHRAG_API_KEY`和`GRAPHRAG_API_BASE`环境变量。
- 依赖管理
  - 采用分层依赖管理策略，M1/M2/M5/UI 层分别对应不同的功能模块。
- 环境检查
  - 提供完整的环境检查工具，自动检测 Python 版本、平台兼容性和依赖完整性。
- Conda 环境配置
  - MetaUrban 提供专用的 Conda 环境配置，包含必要的编译工具和依赖项。

**章节来源**
- [metaurban/environment.yml:1-16](file://metaurban/environment.yml#L1-L16)
- [metaurban/setup.py:1-130](file://metaurban/setup.py#L1-L130)
- [scripts/m1_00_check_env.py:1-79](file://scripts/m1_00_check_env.py#L1-L79)
- [scripts/test_llm_api.sh:14-26](file://scripts/test_llm_api.sh#L14-L26)

### L. 自动化测试指导
- 测试框架
  - 使用 pytest 作为主要测试框架，支持参数化测试和 fixtures。
- 测试分类
  - 单元测试：验证独立函数和类的行为，使用 monkeypatch 进行依赖注入。
  - 集成测试：验证模块间的协作，使用 TestClient 进行 API 测试。
  - 端到端测试：覆盖完整的业务流程，模拟真实用户场景。
  - **新增** LLM API测试：使用`scripts/test_llm_api.sh`验证LLM API连通性和配置。
  - **新增** 评估工具测试：使用 mock LLM 模式验证 snapshot-diff 管道的完整流程。
- 测试数据管理
  - 使用临时目录进行测试数据隔离，确保测试的可重复性。
  - 通过 monkeypatch 替换外部依赖，提高测试稳定性。
- 测试覆盖率
  - 重点关注核心算法和关键业务流程的测试覆盖。
  - 对异常路径和边界条件进行充分测试。
  - **新增** LLM API测试覆盖率：确保API配置、模型列表和响应格式的测试覆盖。
  - **新增** 评估工具测试覆盖率：确保配置差异计算、HTML 报告生成、评分图表绘制等功能的测试覆盖。

**章节来源**
- [tests/test_m1_pipeline.py:1-219](file://tests/test_m1_pipeline.py#L1-L219)
- [tests/test_m2_pipeline.py:1-261](file://tests/test_m2_pipeline.py#L1-L261)
- [tests/test_m3_street_compose.py:1-800](file://tests/test_m3_street_compose.py#L1-L800)
- [tests/test_snapshot_diff.py:1-302](file://tests/test_snapshot_diff.py#L1-L302)
- [scripts/test_llm_api.sh:1-68](file://scripts/test_llm_api.sh#L1-L68)

### M. LLM API测试工具使用指南
- **新增** 工具概述
  - `scripts/test_llm_api.sh`提供完整的LLM API连接性测试功能。
  - 支持基本API连通性测试、模型列表查询和错误诊断。
  - 自动加载项目根目录的`.env`文件进行配置。
- **新增** 使用方法
  - 基本用法：`./scripts/test_llm_api.sh`（使用默认模型）
  - 指定模型：`./scripts/test_llm_api.sh gpt-4o-mini`
  - 列出模型：`./scripts/test_llm_api.sh --list`
- **新增** 配置要求
  - 需要在项目根目录创建`.env`文件，包含以下配置：
    - `GRAPHRAG_API_KEY=your_api_key`
    - `GRAPHRAG_API_BASE=https://api.zetatechs.com/v1/`
- **新增** 输出信息
  - 显示API端点和使用的模型
  - 输出HTTP状态码和响应内容
  - 成功时显示"API is working"，失败时显示错误信息

**章节来源**
- [scripts/test_llm_api.sh:1-68](file://scripts/test_llm_api.sh#L1-L68)

### N. snapshot-diff 自动化评估工具使用指南
- **新增** 工具概述
  - snapshot-diff 脚本提供完整的自动化评估流程，支持单查询的迭代评估和可视化报告生成。
  - 适用于快速验证设计草稿的质量、跟踪迭代改进效果和生成可分享的评估报告。
- **新增** 使用方法
  - 基本用法：`make snapshot-diff`
  - 自定义查询：`SNAPSHOT_QUERY="现代步行友好街道" make snapshot-diff`
  - 自定义迭代次数：`SNAPSHOT_ITERS=5 make snapshot-diff`
  - 离线模式：`SNAPSHOT_QUERY="现代步行友好街道" SNAPSHOT_ITERS=3 make snapshot-diff`
- **新增** 输出结构
  - 生成时间戳命名的输出目录，包含迭代目录、最终结果、差异分析和报告文件。
  - 每个迭代包含配置补丁、评估结果、场景布局和预览图像。
  - diffs 目录包含配置差异 JSON 文件和图像对比文件。
  - 顶层生成 HTML 报告、JSON 评估报告和迭代日志。
- **新增** 配置参数
  - 查询参数：设计查询文本，默认为"modern pedestrian-friendly street with trees and benches"
  - 最大迭代数：默认 3 次迭代
  - 资产清单：默认使用 data/street_furniture/street_furniture_manifest.jsonl
  - CLIP 模型：默认 models/clip-vit-base-patch32
  - 设备选择：默认 CPU，支持 GPU 加速
  - 离线模式：--local-files-only 参数启用离线模式

**章节来源**
- [scripts/snapshot_diff.py:1-608](file://scripts/snapshot_diff.py#L1-L608)
- [Makefile:94-107](file://Makefile#L94-L107)

### O. 街道家具清单增强工具使用指南
- **新增** 工具概述
  - enrich_street_furniture_manifest.py 用于从原始清单创建标准化的街道家具清单。
  - 自动添加文本描述、占位符潜在文件、分类标准化等字段，提升数据质量和可用性。
- **新增** 使用方法
  - 基本用法：`python scripts/enrich_street_furniture_manifest.py`
  - 指定输入文件：`python scripts/enrich_street_furniture_manifest.py --input data/street_furniture/raw_manifest.jsonl`
  - 指定输出文件：`python scripts/enrich_street_furniture_manifest.py --output data/street_furniture/enriched_manifest.jsonl`
  - 指定注释文件：`python scripts/enrich_street_furniture_manifest.py --annotation data/street_furniture/master_annotation.json`
- **新增** 数据处理流程
  - 分类标准化：将相似的分类归并到统一的标准分类（如 bench、lamp、trash 等）。
  - 文本描述生成：基于注释信息和标签生成详细的文本描述。
  - 占位符潜在文件：为每个资产创建占位符潜在文件，支持后续的潜在向量计算。
  - 质量评估：计算网格面数，生成质量注释，标注场景适用性。
- **新增** 输出字段
  - text_desc：标准化的文本描述
  - latent_path：占位符潜在文件路径
  - category：标准化分类
  - split：数据分割标识（默认 train）
  - latent_source：潜在向量来源
  - mesh_face_count：网格面数
  - quality_notes：质量评估注释

**章节来源**
- [scripts/enrich_street_furniture_manifest.py:1-195](file://scripts/enrich_street_furniture_manifest.py#L1-L195)

### P. Makefile 中 snapshot-diff 目标配置详解
- **新增** 目标配置
  - 目标名称：snapshot-diff
  - 依赖：scripts/snapshot_diff.py
  - 环境变量：
    - SNAPSHOT_QUERY：默认查询文本
    - SNAPSHOT_ITERS：默认迭代次数
    - SF_MANIFEST：默认资产清单路径
- **新增** 命令执行
  - 调用 Python 解释器执行 snapshot_diff.py 脚本
  - 传递查询参数、迭代次数、资产清单路径、CLIP 模型路径、离线模式和设备参数
  - 支持环境变量覆盖默认配置
- **新增** 使用示例
  - 基本使用：`make snapshot-diff`
  - 自定义查询：`SNAPSHOT_QUERY="现代步行友好街道" make snapshot-diff`
  - 自定义迭代：`SNAPSHOT_ITERS=5 make snapshot-diff`
  - 离线模式：`SNAPSHOT_QUERY="现代步行友好街道" SNAPSHOT_ITERS=3 make snapshot-diff`

**章节来源**
- [Makefile:94-107](file://Makefile#L94-L107)

### Q. 评估工具与数据处理的最佳实践
- **新增** snapshot-diff 使用最佳实践
  - 选择合适的查询文本，确保设计意图清晰明确。
  - 根据硬件性能调整迭代次数，平衡评估质量和计算时间。
  - 使用离线模式进行离线评估，避免网络依赖。
  - 定期清理生成的中间文件，控制磁盘空间使用。
- **新增** 街道家具清单处理最佳实践
  - 建立标准化的分类映射表，确保分类的一致性。
  - 定期更新注释文件，保持分类信息的准确性。
  - 使用占位符潜在文件机制，支持后续的数据增强和质量评估。
  - 建立质量评估标准，定期审查和更新质量注释。
- **新增** LLM API测试最佳实践
  - 在`.env`文件中集中管理API配置，避免硬编码。
  - 使用`--list`参数定期检查可用模型，确保配置正确。
  - 在CI/CD环境中使用离线模式进行测试，避免网络依赖。
  - 建立API连接性监控，及时发现服务中断问题。

**章节来源**
- [scripts/snapshot_diff.py:1-608](file://scripts/snapshot_diff.py#L1-L608)
- [scripts/enrich_street_furniture_manifest.py:1-195](file://scripts/enrich_street_furniture_manifest.py#L1-L195)
- [scripts/test_llm_api.sh:1-68](file://scripts/test_llm_api.sh#L1-L68)