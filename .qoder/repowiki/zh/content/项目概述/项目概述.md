...
- [src/roadgen3d/design_rules.py](file://src/roadgen3d/design_rules.py)
</cite>

## 更新摘要
**所做更改**
- 新增综合技术概述文档 `docs/about.md` 的分析和整合
- 更新自动化流水线架构和评估机制的详细说明
- 增强迭代优化过程和多层评估系统的描述
- 更新LLM设计助手和RAG集成的技术细节
- 新增POI约束规则和设计规则配置的完整分析
- 更新技术栈和架构图示，反映最新的自动化能力
...
- **自动化流水线**: 完整的端到端自动化场景生成和迭代优化流程

**章节来源**
- [readme.md:1-66](file://readme.md#L1-L66)
- [docs/about.md:11-31](file://docs/about.md#L11-L31)
...
subgraph "自动化流水线"
AUTO_PIPE["src/roadgen3d/auto_pipeline/*"]
END
RG3D --> WEB_API
RG3D --> UI_API
WEB_API --> WORKBENCH
...
### 自动化流水线（新增）
- **AutoIterationController**: 实现生成→渲染→评估→改进的循环
- **GraphLoader**: 解析查看器导出的图JSON，提取场景生成上下文
- **SceneRenderer**: 生成自上而下的场景预览图
- **多层评估系统**: 工程指标 + LLM视觉评估 + 规则合规性评估
...
- [docs/about.md:117-142](file://docs/about.md#L117-L142)
...
- [docs/about.md:25-36](file://docs/about.md#L25-L36)
...
#### 工程指标评估（eval_metrics.py）
- `compute_overlap_rate()` - AABB碰撞检测
- `compute_dropped_slot_rate()` - 失败放置跟踪
- `compute_spacing_uniformity()` - 分布均匀性（变异系数）
- `compute_style_consistency()` - CLIP分数
- `compute_balance_score()` - 左右对称性
- `compute_rule_satisfaction_rate()` - 设计规则合规率
- `compute_topology_validity()` - 网络拓扑
- `compute_cross_section_feasibility()` - 横截面验证

#### LLM视觉评估（design_workflow.py + prompts.py）
- 支持图像预览的视觉场景评估
- 输出：0-10分的评分、建议、配置补丁
- 评估视觉美学、空间布局、多样性、合规性、行人友好度

#### POI约束规则（poi_rules.py）
- `entrance_clearance()` - 4米入口开口（60%角度）
- `fire_access()` - 消防车通道
- `bus_stop_clearance()` - 公交站通道
- `crossing_keep_clear()` - 交叉口视线
- `traffic_signal_visibility()` - 交通信号可见性
- `parking_entrance_clearance()` - 停车入口通道

#### 设计规则配置（design_rules.py）
- `BALANCED_RULES` - 平衡完整街道配置文件
- `PEDESTRIAN_RULES` - 行人优先配置文件
- 硬性规则（必须满足）vs 软性规则（惩罚）

#### 合规性评估（compliance_eval.py）
- 场景级合规评分
- 批量合规评估
- 违规跟踪和约束惩罚

**章节来源**
- [docs/about.md:40-78](file://docs/about.md#L40-L78)
- [src/roadgen3d/eval_metrics.py:13-146](file://src/roadgen3d/eval_metrics.py#L13-L146)
- [src/roadgen3d/compliance_eval.py:14-55](file://src/roadgen3d/compliance_eval.py#L14-L55)
- [src/roadgen3d/poi_rules.py:77-198](file://src/roadgen3d/poi_rules.py#L77-L198)
- [src/roadgen3d/design_rules.py:9-407](file://src/roadgen3d/design_rules.py#L9-L407)

### 迭代优化过程（新增）

AutoIterationController实现了自动化的场景优化循环：

```python
# iteration_controller.py
MAX_ITERATIONS = 5
EARLY_STOP_THRESHOLD = 2  # 连续无改进轮次
...
**章节来源**
- [docs/about.md:81-114](file://docs/about.md#L81-L114)
...
- **自动化流水线**: 检查图JSON格式和基础地图数据URL；验证迭代次数和阈值设置
...
- **自动化流水线**: 完整的端到端自动化场景生成和迭代优化流程
- **多层评估系统**: 工程指标 + LLM视觉评估 + 规则合规性评估的综合质量保证

**章节来源**
- [readme.md:422-441](file://readme.md#L422-L441)
- [docs/about.md:246-264](file://docs/about.md#L246-L264)
...
- **自动化流水线**: 完整的端到端场景生成和优化流程
- **多层评估系统**: 工程指标 + LLM视觉评估 + 规则合规性评估

#### 自动化流水线优势
- **智能迭代**: 基于LLM反馈的自动参数调整
- **多维度评估**: 综合考虑工程性能、视觉质量和设计合规性
- **早停机制**: 避免无效的多次迭代
- **缓存优化**: 减少重复计算和LLM调用成本

**更新** 技术栈现在包含了现代化的LLM集成和完整的自动化流水线，使用新的环境变量命名约定，提供更好的向后兼容性和用户体验。
...
**更新** 路线图现在包含了LLM配置标准化的目标和自动化流水线优化的计划，强调完全采用'llm_base_url'命名约定。
...
**图表来源**
- [docs/about.md:145-242](file://docs/about.md#L145-L242)
