...
## 更新摘要
**变更内容**
- 新增自动FAISS索引构建功能，通过`_ensure_index`函数实现智能索引管理
- 增强了资产检索系统的自动化能力，支持从manifest文件自动构建CLIP+FAISS索引
- 添加了智能检查现有索引文件、错误处理和日志记录等功能
- 扩展了检索流程，支持在运行时动态构建索引
...
- 自动索引构建：智能检查现有索引文件、自动构建CLIP+FAISS索引
...
- 自动索引构建：智能检查现有索引文件，必要时自动构建CLIP+FAISS索引
...
- [snapshot_diff.py:66-109](file://scripts/snapshot_diff.py#L66-L109)
...
- 自动索引构建器：智能检查现有索引文件，必要时从manifest文件自动构建CLIP+FAISS索引
...
## 架构总览
资产检索系统采用"离线嵌入+在线索引"的两阶段设计，现已增强为"智能索引构建+在线检索"的自动化架构：
- 离线阶段：读取资产描述，使用CLIP编码为向量并保存；构建FAISS索引并持久化
- 在线阶段：接收文本查询，编码后在FAISS中进行Top-K检索，返回资产ID与分数；根据ID加载潜变量并解码
- 自动化阶段：智能检查索引文件存在性，必要时自动从manifest构建索引

```mermaid
sequenceDiagram
participant U as "用户"
participant SD as "snapshot_diff.py"
participant EI as "_ensure_index()"
participant E as "ClipTextEmbedder"
participant I as "FaissIndexStore"
participant L as "LatentStore"
U->>SD : 提交文本查询
SD->>EI : 检查索引文件存在性
EI->>EI : 智能检查index_ip.faiss与id_map.json
alt 索引不存在
EI->>E : 从manifest加载文本描述
E-->>EI : 返回文本向量
EI->>I : 构建FAISS索引
I-->>EI : 返回已保存索引
end
...
**图表来源**
- [snapshot_diff.py:338-494](file://scripts/snapshot_diff.py#L338-L494)
- [snapshot_diff.py:66-109](file://scripts/snapshot_diff.py#L66-L109)
...
### 智能索引检查机制
新的`_ensure_index`函数实现了智能索引检查机制，能够在运行时自动判断是否需要构建FAISS索引：
- 文件存在性检查：检查`index_ip.faiss`和`id_map.json`是否存在
- 条件构建：只有当索引文件不存在时才执行构建流程
- 目录管理：自动创建必要的输出目录结构

### 自动构建流程
当索引文件不存在时，`_ensure_index`函数执行完整的构建流程：
- Manifest读取：从JSONL文件逐行解析资产描述
- 文本编码：使用CLIP模型对所有资产描述进行批量编码
- 索引构建：创建FaissIndexStore并保存索引文件
- 日志记录：提供详细的构建进度和统计信息

```mermaid
flowchart TD
A["_ensure_index() 调用"] --> B["检查 index_ip.faiss 存在性"]
B --> C["检查 id_map.json 存在性"]
C --> D{两个文件都存在?}
D --> |是| E["直接返回"]
D --> |否| F["创建输出目录"]
F --> G["读取 manifest.jsonl"]
G --> H["提取 asset_ids 和 text_desc"]
H --> I["使用 CLIP 编码文本"]
I --> J["构建 FaissIndexStore"]
J --> K["保存索引文件"]
K --> L["打印构建完成信息"]
```
...
### 集成到主流程
`_ensure_index`函数被集成到`run_snapshot_pipeline`主流程中：
- 位置：在执行AutoIterationController之前调用
- 目标：确保共享的`_shared_artifacts`目录中存在有效的FAISS索引
- 参数：接收manifest路径、输出目录、模型配置等参数
- 返回：无返回值，直接修改文件系统
...
- 自动索引依赖：新增对JSONL格式manifest文件的解析支持
...
S0 --> F
SD["snapshot_diff.py"] --> E["ClipTextEmbedder"]
SD --> I["FaissIndexStore"]
SD --> M["manifest.jsonl"]
```
...
- 自动索引失败：检查manifest文件格式，确保包含`asset_id`和`text_desc`字段
- 索引损坏：删除损坏的索引文件，重新运行构建流程
...
## 结论
资产检索系统通过CLIP文本嵌入与FAISS向量检索，实现了高效、稳定的文本到3D资产匹配。新增的自动索引构建功能进一步增强了系统的自动化能力，能够智能检查并构建FAISS索引，减少了手动维护成本。系统具备清晰的离线-在线分离架构、完善的错误处理与环境检查机制，并提供了可扩展的类型定义与解码接口。遵循本文档的性能优化与最佳实践，可在不同硬件环境下获得稳定且高效的检索体验。
...
- 自动索引构建：智能检查索引文件，必要时自动构建
  - 示例路径：[snapshot_diff.py:66-109](file://scripts/snapshot_diff.py#L66-L109)
