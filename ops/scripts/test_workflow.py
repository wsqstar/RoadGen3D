...
def _adapt_preset(preset: dict) -> dict:
    """Adapt shared preset format to test workflow format."""
    return {
        "id": preset["id"],
        "name": preset["name"],
...
SCENE_PRESETS_WORKFLOW = [_adapt_preset(p) for p in SCENE_PRESETS]
...
Examples:
  ...
  uv run python scripts/test_workflow.py --api-base http://127.0.0.1:8010 --timeout 600
    ...
    parser.add_argument(
        "--preset",
        choices=[p["id"] for p in SCENE_PRESETS_WORKFLOW],
        default=None,
    ...
    # Select preset
    if args.preset:
        preset = next(p for p in SCENE_PRESETS_WORKFLOW if p["id"] == args.preset)
    else:
        preset = random.choice(SCENE_PRESETS_WORKFLOW)
