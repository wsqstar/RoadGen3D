...
Status: current method note; automatic evaluation source of truth is `data/scenario_designs/hkust_gz_gate_evaluation_rubric.json`.  
...
自动化实现已落到 `ScenarioRubricEvaluator`：默认 Pass / Review / Fail 只使用可复现的结构化指标和 semantic gates；LLM/截图视觉评价只作为解释性 evidence，不参与 v1 自动判定。本文阈值仍标记为 `heuristic-to-calibrate`，后续需要用专家评分或 benchmark 样本分布校准。
