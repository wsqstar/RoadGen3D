...
app.innerHTML = `
  <div class="shell">
    <section class="hero">
      <h1>RoadGen3D Complete Street Workbench</h1>
      <p>
        用对话确认街道目标，让 LLM 生成 RAG 查询，从完整街道设计指南中抽取证据，再把可编辑的参数草案送入现有 RoadGen3D 生成链路。
      </p>
      <div class="hero-grid">
        ...
        <div class="hero-chip">4. Generate + Viewer</div>
      </div>
    </section>
      ...
      <div class="stack">
        <section class="panel">
          <div class="panel-head">
            <h2>Conversation</h2>
            <span id="api-status" class="tag">API: ${API_BASE}</span>
          </div>
...
const timelineEl = requireElement<HTMLDivElement>("#timeline");
const promptInput = requireElement<HTMLTextAreaElement>("#prompt-input");
const draftBtn = requireElement<HTMLButtonElement>("#draft-btn");
const rebuildBtn = requireElement<HTMLButtonElement>("#rebuild-btn");
const statusBox = requireElement<HTMLDivElement>("#status-box");
const intentBox = requireElement<HTMLDivElement>("#intent-box");
const evidenceList = requireElement<HTMLDivElement>("#evidence-list");
const draftSummary = requireElement<HTMLDivElement>("#draft-summary");
const parameterForm = requireElement<HTMLDivElement>("#parameter-form");
const generateBtn = requireElement<HTMLButtonElement>("#generate-btn");
const generationResult = requireElement<HTMLDivElement>("#generation-result");
...
generateBtn.addEventListener("click", async () => {
  ...
  try {
    const draft = buildDraftFromForm(state.lastDraft.draft);
    ...
    setStatus("生成完成，可以打开 viewer 查看结果。");
  } catch (error) {
    setStatus(asErrorMessage(error));
  } finally {
...
function buildDraftFromForm(baseDraft: DesignDraft): DesignDraft {
...
function renderGeneration(result: GenerationResponse): void {
  const links = [
    result.viewer_url ? `<div><a href="${escapeHtml(result.viewer_url)}" target="_blank" rel="noreferrer">Open Viewer</a></div>` : "",
