...
> 归档更新：`web/workbench` 已保留为 legacy 参考实现，默认不再启动；当前交互式生成、溯源、生长树、结果展示与评价入口统一迁入 `web/viewer`。

## 1. 结论摘要

当前项目已经形成了比较清晰的“三层式”结构：

1. `web/viewer` 是当前主前端，负责设计输入、生成、溯源、生长树、评估、Three.js 可视化、注释、资产编辑和对比查看。
2. `web/workbench` 是已归档的旧 React 前端，源码保留用于迁移追溯，默认不启动。
