...
> 一个神经符号街道场景生成与评价框架。它将设计意图、图模板、OSM/参考图上下文和规则 profile 转换为可解释、可评价、可迭代改进的 3D 街道场景。

它已经覆盖街道空间设计、横断面组织、街道家具/建筑布置、场景输出、评价和 Viewer 展示。它尚未完整覆盖道路工程级的 lane movement、信号控制、交通仿真、道路规范库和生产级任务调度。

## 2. 主流程

当前产品主流程以 `web/viewer` 为交互入口，以 `web/api/main.py` 为业务后端，以 `compose_street_scene()` 为生成核心。
...
用户输入
  ...
  - preset
  - graph template
  - reference annotation / OSM context
  ↓
Viewer
  - web/viewer/src/viewer-design-controller.ts
  - web/viewer/src/viewer-design.ts
  - web/viewer/src/viewer-branch-workspace.ts
  ↓
FastAPI
  - web/api/main.py
  - /api/design/draft
...
Generation runtime
  ...
  - graph_template / metaurban / osm routing
  ↓
Core generation
...
| Neuralsymbolic IR | `types.py`、`street_program.py`、`design_rules.py`、`layout_solver.py` | `StreetProgram`、`ConstraintSet`、`LayoutSolverResult` 等中间表示 |
...
| 输入 | 当前支持 | 说明 |
| --- | --- | --- |
| 自然语言 prompt | 支持 | 通过 LLM/RAG 生成 `DesignDraft`；也可在 Viewer 中直接作为 draft query |
...
RoadGen3D 的核心价值在于不直接从 prompt 跳到网格，而是经过显式中间表示：

1. `DesignDraft`：LLM/RAG 或 preset 生成的设计草案。
...
5. 补 lane-level connectivity、turn movement、signal/control model 后，再提升“道路工程框架”的表述强度。
