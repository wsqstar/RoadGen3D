...
- 新增 `analytical_diorama` lighting preset。
- 默认 Viewer lighting 切到 `analytical_diorama`。
- 加载 GLB 后遍历 mesh material：
  - 降低过高饱和度。
  - 统一提高 roughness。
  - 限制非金属物体的 metalness。
...
- 使用 footprint + height 生成建筑体块。
- 近景建筑生成程序化窗格和首层界面。
- 远景建筑简化为低饱和 massing。
- 将建筑标记为 background layer，避免抢街道主体。

### Phase 3: Diagrammatic Ground System

Goal: 让地面从“贴图地面”变成“3D 设计图”。

- 对 road / sidewalk / bike lane / furnishing / planting 使用稳定 role palette。
- 增加轻微 noise / normal / tile scale，避免纯色玩具感。
- crosswalk、lane marking、curb edge 保持清晰。

### Phase 4: Analysis Overlay

Goal: 把 Benchmark Explorer 的解释能力投射回 3D 场景。

- 点击 Pareto 点时，在场景中高亮 active features。
- `sidewalk_width_m` 高亮 sidewalk bands。
- `tree_count` 高亮树列。
- `safety` 相关高亮 lighting / bollard / crossing。
...
- Add Viewer `analytical_diorama` lighting preset.
- Apply a lightweight analytical diorama material finish after GLB load.
- Keep the transformation view-only; do not mutate `scene_layout.json` or source GLB files.
