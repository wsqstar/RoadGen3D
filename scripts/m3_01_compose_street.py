...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--sidewalk-knowledge-dir",
        type=Path,
        default=None,
        help="Optional path to knowledge/sidewalk_area assets.",
    )
    parser.add_argument(
        "--no-sidewalk-constraints",
        action="store_true",
        help="Disable sidewalk knowledge checks even if a directory is provided.",
    )
...
def main() -> int:
    ...
    config = StreetComposeConfig(
        ...
        sidewalk_knowledge_dir=str(args.sidewalk_knowledge_dir) if args.sidewalk_knowledge_dir else None,
        sidewalk_constraints_enabled=not bool(args.no_sidewalk_constraints),
    )
    try:
