"""FastAPI entrypoint for the LLM + RAG design assistant."""
...
def create_app(*, design_service: DesignAssistantService | Any | None = None) -> FastAPI:
    app = FastAPI(title="RoadGen3D Design Assistant API", version="0.1.0")
    app.add_middleware(
        CORSMiddleware,
        allow_origins=["*"],
        ...
        try:
            result = service.generate_scene(
                draft=parse_design_draft(
                    request.draft,
                    evidence=(),
                    fallback_query=str(request.draft.get("normalized_scene_query", "") or ""),
                    current_patch=request.draft.get("compose_config_patch", {}) or {},
                ),
